...
| §8 the fault-kind enum is **closed** | §6.5.1 — closed, and its membership **relocated here and extended to seven**; #18 §8's five are a superseded prefix. The two extensions (ADR-0015 §4, ADR-0048 §7) are named as members 6 and 7. Gap ch5 §13.1 **closed**, [#113](https://github.com/ludo-lang/ludo/issues/113) |
...
| §4 asset resolution failure is a spec-named fault kind carrying location and path | §8.4, §6.1.4, §6.5.1 item 6 — the first of the two extensions ch5 §13.1 recorded, now a named member of the closed enum |
...
| ADR-0048 §7 allocation failure in a `persist` initialiser is a fault; the report names the declaration; the carve-out is by position | §4.6.1, §4.6.2, §4.6.3, §6.1.5, §6.5.1 item 7 — the **implicit** second extension of #18 §8's closed enum, made explicit as the kind **allocation failure** ([#113](https://github.com/ludo-lang/ludo/issues/113)) |
...
- **The fault-kind enum's membership** is not an induced hole — it was opened by
  **reading** ADR-0015 §4 against ADR-0048 §7 against #18 §8, which is the
  ...
  and named here only so it is not miscounted as one. **Now closed**: §6.5.1
  carries the seven-member list, §6.1.7 relocates the closure off an issue, and
  §6.1.8 excludes the undetected `unsafe` use-after-free from it.
