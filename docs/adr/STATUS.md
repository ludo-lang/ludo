...
ADR-0054 | live | a backend is a build input the toolchain supplies, not a library — it claims no root name, is no key in the mapping, and is selected whole as the platform-layer triple | reverses:0014(§9's root-name clause only), amends:0019(§1's spec-defined-root phrase, closed as loose wording), upholds:0006(R3), rests-on:0027(§1 duplicate-key rejection carries exclusivity), closes:#112, zero #24 delta
```
